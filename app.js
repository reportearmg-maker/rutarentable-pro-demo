...
function summary(sessions) {
  return sessions.reduce((a,s) => {
    ...
    indriveRecharge:0, rechargePending:0, appExpenses:0
  });
}
...
function renderReports() {
  ...
  document.getElementById("reportSessions").innerHTML = [...sessions].sort((a,b)=>new Date(b.endDate)-new Date(a.endDate)).map(x => {
    ...
    return `<tr><td>${formatDate(x.endDate)}</td><td>${escapeHtml(getVehicleName(x.vehicleId))}</td><td>${h.toFixed(1)}</td><td>${km}</td>
      <td>${money(sessionIncome(x))}</td><td>${money(getInDriveCommission(x))}</td><td>${money(sessionExpenses(x))}</td>
...
function calculateFinishPreview() {
  ...
  const expenses = Number(active.startFuel||0)
    ...
    + Number(document.getElementById("finishTolls").value||0)
...
function exportSessionsCsv() {
  const headers=[
    ...
    "Corredores y peajes","Otros gastos de aplicaciones","Otros gastos generales",
    "Total cobrado","Total gastos","Ganancia neta","Horas","Ganancia por hora","Ganancia por km"
  ];
