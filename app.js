...
function renderVehicles() {
  ...
  grid.innerHTML = state.vehicles.map(v => {
    ...
    const s = summary(sessions);
    return `<article class="panel vehicle-card"><h3>${escapeHtml(v.name)}</h3>
...
document.addEventListener("click",e=>{
  ...
  if(del){state.sessions=state.sessions.filter(s=>s.id!==del.dataset.deleteSession);saveState();renderAll();toast("Jornada eliminada.");}
});
